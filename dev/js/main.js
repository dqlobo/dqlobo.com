...
function Bubble(name, highlightedColor, link=undefined, radius=50) {
    ...
    this.point = new Point(getRandomArbitrary(0, window.innerWidth),getRandomArbitrary(0, window.innerHeight));
    this.radius = 50;
	...
	// ctx.stroke();
	ctx.fill();
	ctx.drawImage(this.image, this.point.x - this.image.width/2, this.point.y - this.image.height/2);
    }
...
function BubbleCanvas() {
	...
	this.getContext().clearRect(0, 0, this.canvasElement.width, this.canvasElement.height);
    }
...
function transformPerspective(event) {
    ...
    $('.content').css(prefixStyles({'transform': 'perspective(1000px) rotateY('+amount+'deg)'}));
    $('.content').css({'box-shadow': -amount*3+'px 20px 25px rgba(0,0,0,0.5)'});
}
